...
path = os.path.join(os.path.dirname(__file__), "load_stance.py")
with open(path, "rb") as f:
    exec(f.read())

df_emb = df[["id"]].merge(df_emb, on="id", how="left")
df_emb = df_emb.reset_index(drop=True)

sbert_embeddings = np.array(
    df_emb[[c for c in df_emb.columns if re.match("^e\w*[0-9]", c)]].values
).astype("float")
