...
from sig_networks.seqsignet_bilstm import SeqSigNet
...
def obtain_SeqSigNet_input(
    ...
    """
    Function to obtain the input for a SeqSigNet model
    (e.g. SeqSigNet, SeqSigNetAttentionEncoder).
    ...
    print(
        f"given shift {shift}, window size {window_size} and n {n}: "
        f"history length = {k}"
    )
...
def implement_seqsignet(
    ...
    and train and evaluates a SeqSigNet model.

    If k_fold=True, it will evaluate the SeqSigNet by performing
    k-fold validation with n_splits number of folds (i.e. train and test
    n_split number of SeqSigNet models), otherwise, it will
    evaluate by training and testing a single SeqSigNet on one
    ...
    -------
    tuple[SeqSigNet, pd.DataFrame]
        SeqSigNet object (if k-fold, this is a randomly
...
def seqsignet_hyperparameter_search(
    ...
    """
    Performs hyperparameter search for the SeqSigNet model for different
    ...
    evaluating a SeqSigNets on various seeds and averaging performance
    over the seeds.
    ...
    will average over the performance of the SeqSigNet
